...
Source: cricket_007, https://stackoverflow.com/questions/36021526/converting-an-array-dict-to-xml-in-python
Modify: JonYonv 1943
"""
...
class TensorFlowXmlIO(object):
    ...
    def update_img_shape(self, img_name):
        ...
        self.tf_xml[self.root]['size'] = {'width': str(self.width), 'height': str(self.height),
    ...
    def csv_to_xml(self, csv_pwd):
        ...
        img_l = [f for f in os.listdir(self.dir_pwd) if f[-4:] == '.jpg']
        for i, csv_result in enumerate(csv_l):
            csv_result_l = csv_result.split(',')
            img_name = csv_result_l[0]

            if not os.path.exists(os.path.join(self.dir_pwd, img_name)):
                print('miss:', img_name)
                continue
            else:
                print(i, os.path.join(self.dir_pwd, img_name))
...
if __name__ == '__main__':
    file_pwd = 'tf_origin'
    csv_pwd = os.path.join(file_pwd, '0_image_info_for_xml.csv')
